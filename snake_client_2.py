    ...
    def start_game(self):
                        ...
                        for i in snakes_list:
                            if self != i:
                                if self.body[0] in i.body[0:-1:1]:
                                    if self.body[0] == i.body[
                                        0] and self.take_side_tehn() == -i.take_side_tehn():  # ппроверка на взаимное столкновение (ничью)
        ...
        class Apple:
            def __init__(self):
                ...
                self.data = {
                    'host': False,
                    'snake1_pside': snake1.pside,
                }
                self.message = pickle.dumps(self.data)
                socket.send(self.message)
                self.data = socket.recv(1024)
                if not self.data:
                    pass
                self.data = pickle.loads(self.data)
                snake.pside = self.data.get('snake_pside')
                self.coordinates = self.data.get('apple_coords')
                APPLE_SIZE = self.data.get('apple_size')
                print(self.data)
        ...
        snake = Snake([[0, 2], [0, 1], [0, 0]], 'bottom', 2, color=COLORS['SNAKE'], name='ЗЕЛЁНАЯ змеюка')
        snake1 = Snake(
        ...
        # отправка и получение данных с сервера (начало)
        ...
        # отправка и получение данных с сервера (конец)
        ...
        while running:
            ...
            for event in pygame.event.get():
                ...
                #         del snake
                #         del snake1
                #         snake = Snake([[0, 2], [0, 1], [0, 0]], 'bottom', 1, color=COLORS['SNAKE'],
                #                       name='ЗЕЛЁНАЯ змеюка')
                #         snake1 = Snake(
                #             [[WINDOW_WIDTH / SIZE - 1, WINDOW_HEIGTH / SIZE - 3],
                #              [WINDOW_WIDTH / SIZE - 1, WINDOW_HEIGTH / SIZE - 2],
            ...
            snake.pside = data.get('snake_pside')
            apple.coordinates = data.get('apple_coords')
            APPLE_SIZE = data.get('apple_size')
